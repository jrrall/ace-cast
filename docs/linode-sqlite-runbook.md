...
block), and `authelia/configuration.yml` + `authelia/users_database.example.yml` (template).
The real `authelia/users_database.yml` is **generated by `bootstrap.sh`** and gitignored.

> **If you deploy with `bootstrap.sh` (recommended), 11a + 11b are done for you:** it mints
> the four secrets into `.env` (idempotently — re-runs keep them) and, on first run, creates
> an `admin` user with a random password that it **prints once** at the end. Override with
> `ADMIN_USER` / `ADMIN_PASSWORD` / `ADMIN_EMAIL` env vars. The manual steps below are for
> doing it by hand or adding/rotating users later.

### 11a. Set the secrets (manual)
...
### 11b. Add / rotate a user (manual)

Create `deploy/linode/authelia/users_database.yml` from the example, then generate a real
argon2id hash on the box:
