"""All eight voices must independently read the same research and reach review."""
import pytest
...
def test_all_voices_share_research_and_split_budget(settings):
    ...
    llm = FakeLLM([{'cards': [*({'kind': 'prompt', 'text': f'{voice} {i}: ____.'} for i in range(6)), *({'kind': 'answer', 'text': f'{voice} {i}'} for i in range(6))]}
                   for voice in ['dry', 'wild', 'spin', 'petty', 'banned', 'uncle', 'positive', 'intrusive']])
    writers = writing_team(llm, settings)
    results = [w.run(theme) for w in writers]
    assert [len(cards) for cards in results] == [1] * 8
    for index, voice in enumerate(['Deadpan', 'Unhinged', 'PR Spin Doctor', 'Petty Villain', 'Banned From 4chan', 'Hatemonger', 'Toxic Positivity', 'Intrusive Thoughts']):
...
def test_pipeline_reviews_cards_from_all_writers(settings):
    ...
    llm = FakeLLM([
        ...
        *[{'cards': [card]} for card in drafts],
        {'cards': drafts},
        {'verdicts': [{'index': i, 'allowed': True, 'maturity_rating': 1} for i in range(8)]},
    ...
    assert len({call['user'] for call in llm.calls[1:9]}) == 1
    for card in drafts:
        assert card['text'] in llm.calls[9]['user']
    assert summary.generated == 8
    assert len(batch.cards) == 8
    assert batch.cards[-1].writer == "writer.intrusive_thoughts"
...
@pytest.mark.parametrize("total", [8, 9, 10, 11, 16, 32])
...
def test_new_roster_requires_eight_but_legacy_six_can_resume(settings):
