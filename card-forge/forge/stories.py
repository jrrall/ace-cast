...
def prompt_ids(stories):
    """Short exact aliases; retain full identities in checkpoint records."""
    result = {}
    for story in stories:
        alias = story['id'][:18]  # story- plus 12 hex digits
        if alias in result and result[alias]['id'] != story['id']:
            raise ValueError('Story prompt ID collision')
        result[alias] = story
    return result
...
def resolve_themes(data, stories, limit):
    ...
    by_id.update(prompt_ids(stories))
    seen, themes = set(), []
